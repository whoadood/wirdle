...
class Game {
    round = 1;
    status = "idle";
    answer = words[Math.floor(Math.random() * words.length)].trim();
    guesses = [];
}

async function wordLogger() {
    const newGame = new Game();
    console.log(gradient.pastel.multiline(figlet.textSync("WIRDLE", {
        font: "3D Diagonal",
    ...
    const answers = await inquirer.prompt([
        {
            ...
            validate(input) {
                if (input.length !== 5) {
                    console.log(chalk.red(`${chalk.bold(" ERROR")} input must be 5 characters long`));
                }
                else {
                    return true;
                }
...
function check(guess, game, callback) {
    ...
    if (guess === game.answer) {
        return gameover(chalk.green(`${chalk.bold("CONGRADULATIONS")} you guessed the wirdle!`));
    }
    game.round++;
    if (game.round > 6) {
